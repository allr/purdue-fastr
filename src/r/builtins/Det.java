    ...
    @Override public RNode create(ASTNode call, RSymbol[] names, RNode[] exprs) {
        ...
        return new Builtin(call, names, exprs) {
            ...
            @Override public RAny doBuiltIn(Frame frame, RAny[] args) {
                ...
                if (!(xarg instanceof RArray)) { throw RError.getInvalidArgument(ast, "x"); // TODO: S3 dispatch / failure here
                }
                RArray xa = (RArray) xarg;
                int[] dims = xa.dimensions();
                if (dims == null || dims.length != 2) { throw RError.getInvalidArgument(ast, "x"); // TODO: S3 dispatch / failure here
                }
                int n = dims[0];
                if (n != dims[1]) { throw RError.getMustBeSquare(ast, "x"); }
                if (xarg instanceof RComplex) { throw RError.getDeterminantComplex(ast); }
                ...
                if (laINFO.val < 0) { throw RError.getLapackError(ast, laINFO.val, "dgetrf"); }
                if (laINFO.val > 0) { return RDouble.BOXED_ZERO; }
                double res = 1;
                for (int i = 0; i < n; i++) {
                    if (laIPIV[i] != i + 1) {
                        res = -res;
