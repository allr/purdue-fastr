            ...
            return new BuiltIn(call, names, exprs) {
                ...
                @Override public RAny doBuiltIn(Frame frame, RAny[] params) {
                    ...
                    if (dim == null || dim.length != 2) {
                        if (size == 0) { throw RError.getInvalidArgument(ast, paramNames[IX]); }// FIXME: not an R warning
                        if (size != 1) { throw RError.getNonSquareMatrix(ast, paramNames[IX]); }
                        n = 1;
                        dimNN = RArray.SCALAR_DIMENSIONS;
                    } else {
                        n = dim[0];
                        if (dim[1] != n) { throw RError.getNonSquareMatrix(ast, paramNames[IX]); }
                        dimNN = dim;
                    }

                    // check for infinite or missing values
                    for (int i = 0; i < values.length; i++) {
                        if (!RDouble.RDoubleUtils.isFinite(values[i])) { throw RError.getInfiniteMissingValues(ast, paramNames[IX]); }
                    }
