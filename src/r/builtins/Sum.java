...
final class Sum extends CallFactory {
    ...
    private static int maxLogicalSum(ASTNode ast) {
        ...
        ASTNode sumParent = ast.getParent();
        if (res != -1
                && (sumParent instanceof r.nodes.GT || sumParent instanceof r.nodes.GE || sumParent instanceof r.nodes.EQ || sumParent instanceof r.nodes.NE || sumParent instanceof r.nodes.LT || sumParent instanceof r.nodes.LE)) { return res; }
        return -1;
    }
                ...
                if (hasComplex) {
                    ...
                    if (!naRM) {
                        for (; i < size; i++) { // still have to check for NAs
                            int l = v.getLogical(i);
                            if (l == RInt.NA) { return RInt.BOXED_NA; }
                        }
                    }
                    if (res < 0) { // overflow
                        return RInt.BOXED_NA;
