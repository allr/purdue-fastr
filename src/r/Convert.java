...
public class Convert {
    ...
    public static String double2string(double d) {
        if (!RDouble.RDoubleUtils.isNA(d)) {
            ...
            if (!RContext.debuggingFormat()) {
                if (RDouble.RDoubleUtils.fitsRInt(d) && ((((int) d)) == d)) { return int2string((int) d); // a hack to get rid of ".0" in "1.0"
                }
            }
    ...
    public static String int2string(int i) {
        if (i == RInt.NA) { return RString.NA; }
        // FIXME use R rules
        if (!RContext.debuggingFormat()) {
    ...
    public static int string2logical(String s, ConversionStatus warn) {
        if (s != RString.NA) {
            if (s.equals("TRUE") || s.equals("T")) { return RLogical.TRUE; }
            if (s.equals("FALSE") || s.equals("F")) { return RLogical.FALSE; }
            if (s.equals("True") || s.equals("true")) { return RLogical.TRUE; }
            if (s.equals("False") || s.equals("false")) { return RLogical.FALSE; }
            if (warn != null) {
                warn.naIntroduced = true;
            }
        }
    ...
    public static byte string2raw(String s, ConversionStatus warn) {
        if (s != RString.NA) {
            ...
            if (intVal >= 0 && intVal <= 255) { return (byte) intVal; }
        }
        if (warn != null) {
    ...
    public static int double2logical(double d) {
        if (RDouble.RDoubleUtils.isNAorNaN(d)) { return RLogical.NA; }
    ...
    public static int int2logical(int i) {
        if (i == RInt.NA) { return RLogical.NA; }
    ...
    public static byte logical2raw(int v, ConversionStatus warn) {
        if (v != RLogical.NA) { return (byte) v; }
        if (warn != null) {
            warn.outOfRange = true;
        }
        return RRaw.ZERO;
    ...
    public static int scalar2int(RAny v) { // FIXME: rewrite to scalar impl types if we have reliable scalarization, or remove
        if (v instanceof RInt) { return ((RInt) v).getInt(0); }
        if (v instanceof RDouble) { return double2int(((RDouble) v).getDouble(0)); }
        if (v instanceof RLogical) { return logical2int(((RLogical) v).getLogical(0)); }
    ...
    public static double scalar2double(RAny v) { // FIXME: rewrite to scalar impl types if we have reliable scalarization, or remove
        if (v instanceof RInt) { return double2int(((RInt) v).getInt(0)); }
        if (v instanceof RDouble) { return ((RDouble) v).getDouble(0); }
        if (v instanceof RLogical) { return logical2double(((RLogical) v).getLogical(0)); }
        Utils.nyi("unsupported type");
        return -1;
    }
