...
expr_wo_assign
	...
	| repeat_expr
	| function
	| NEXT ((LPAR)=>LPAR n_ RPAR)? -> NEXT
	| BREAK ((LPAR)=>LPAR n_ RPAR)? -> BREAK
	;
sequence
...
alter_assign
	: l=tilde_expr	
		( (ARROW)=>ARROW n_ r=expr_or_assign -> ^(ARROW $l $r)
		| (SUPER_ARROW)=>SUPER_ARROW n_ r=expr_or_assign -> ^(SUPER_ARROW $l $r)
		| (RIGHT_ARROW)=>a=RIGHT_ARROW n_ r=expr_or_assign -> ^(ARROW[$a] $r $l)
		| (SUPER_RIGHT_ARROW)=>a=SUPER_RIGHT_ARROW n_ r=expr_or_assign -> ^(SUPER_ARROW[$a] $r $l)
		| (ASSIGN)=>a=ASSIGN n_ r=expr_or_assign -> ^(ARROW[$a] $l $r)
		| -> $l
		)
	;
// $>
...
par_decl
	...
	;
tilde_expr
	: or_expr ((TILDE)=>TILDE^ n_! or_expr)*
	;
or_expr
	: and_expr ((or_operator)=>or_operator^ n_! and_expr)*
	;
and_expr
	: comp_expr ((and_operator)=>and_operator^ n_! comp_expr)*	
	;
comp_expr
	: add_expr ((comp_operator)=>comp_operator^ n_! add_expr)*	
	;
add_expr
	: mult_expr ((add_operator)=>add_operator^ n_! mult_expr)*
	;
mult_expr
	: operator_expr ((mult_operator)=>mult_operator^ n_! operator_expr)*
	;
operator_expr
	: column_expr ((OP)=>OP^ n_! column_expr)*
	;
column_expr
	: power_expr ((COLUMN)=>COLUMN^ n_! power_expr)*
	;
power_expr
	: l=unary_expression ((power_operator)=>power_operator^ n_! r=unary_expression)* // buggy l'associativite est a droite (?)
	;
unary_expression
...
basic_expr
	: (lhs=simple_expr -> $lhs)
			( (FIELD)=>(FIELD n_ name=id)  -> ^(FIELD $basic_expr $name)
			| (AT)=>(AT n_ name=id) -> ^(AT $basic_expr $name)
			| (LBRAKET)=>(LBRAKET subscript=expr_list RBRAKET) -> ^(BRAKET $basic_expr $subscript?)
			| (LBB)=>(LBB subscript=expr_list RBRAKET RBRAKET) -> ^(LBB $basic_expr $subscript?)
						// Must use RBRAKET instead of RBB beacause of : a[b[1]]
			| (LPAR)=>(LPAR a=args RPAR)   -> ^(CALL $basic_expr $a?)
			)*
	;
simple_expr
	: id
