...
public class Truffleize implements Visitor {
    ...
    @Override public void visit(FunctionCall functionCall) {
        ...
        RNode parentOfValueExpr = valueExpr.getParent();
    ...
    public static boolean isArrayColumnSubset(boolean subset, RNode[] selectors) {
        ...
        if (selectors[selectors.length - 1] == null) { return false; }
        return true;
    }
        ...
        if (sa.convertedExpressions.length >= 2) { // array or matrix ?
            ...
            for (int i = 0; i < nodes.length; i++) {
                if (names[i] == RSymbol.DROP_SYMBOL) {
                    if (drop != null) { throw RError.getIncorrectSubscripts(a); }
                    drop = nodes[i];
            ...
            if (dims == 2) { // if matrix read, use the specialized matrix form
                if (a.isSubset() && selectors[0] == null && selectors[1] != null) { // matrix column
                    result = new ReadArray.MatrixColumnSubset(a, createTree(a.getVector()), selectors[1], ReadArray.createDropOptionNode(a, drop), ReadArray.createExactOptionNode(a, exact));
                    return;

                }
                if (a.isSubset() && selectors[0] != null && selectors[1] == null) {
                    result = new ReadArray.MatrixRowSubset(a, createTree(a.getVector()), selectors[0], ReadArray.createDropOptionNode(a, drop), ReadArray.createExactOptionNode(a, exact));
                    return;
                }
                ...
                if (!a.isSubset()) {
                    result = new ReadArray.MatrixSubscript(a, createTree(a.getVector()), selectorIExpr, selectorJExpr, ReadArray.createDropOptionNode(a, drop), ReadArray.createExactOptionNode(a,
                            exact));
                } else {
                    result = new ReadArray.MatrixRead(a, a.isSubset(), createTree(a.getVector()), selectorIExpr, selectorJExpr, ReadArray.createDropOptionNode(a, drop),
            ...
            if (isArrayColumnSubset(a.isSubset(), selectors)) {
                result = new ReadArray.ArrayColumnSubset(a, createTree(a.getVector()), selectors.length, selectors[selectors.length - 1], ReadArray.createDropOptionNode(a, drop),
                        ReadArray.createExactOptionNode(a, exact));
                return;
            }
            ...
            result = new ReadArray.GenericRead(a, a.isSubset(), createTree(a.getVector()), selNodes, ReadArray.createDropOptionNode(a, drop), ReadArray.createExactOptionNode(a, exact));
            return;
        }
        Utils.nyi("unsupported indexing style");
    }
                ...
                if (indexNode instanceof EQ) {
                    ...
                    if (eqNode.getRHS() instanceof Constant) {
                        ...
                        if (cv instanceof RDouble && ((RDouble) cv).size() == 1) {
                            double c = ((RDouble) cv).getDouble(0);
                            if (RDouble.RDoubleUtils.isFinite(c)) {
                                result = new r.nodes.truffle.UpdateVector.LogicalEqualitySelection(a, u.isSuper(), var, createTree(varAccess), createTree(eqNode.getLHS()), c, createTree(u.getRHS()),
                                        a.isSubset());
