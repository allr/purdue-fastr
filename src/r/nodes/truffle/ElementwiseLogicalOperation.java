...
public abstract class ElementwiseLogicalOperation extends BaseR {
    ...
    public static ElementwiseLogicalOperation createUninitialized(ASTNode ast, RNode left, Operation op, RNode right) {
        return new ElementwiseLogicalOperation(ast, left, op, right) {
            ...
            @Override public RAny execute(RAny leftValue, RAny rightValue) {
                try {
    ...
    public static ElementwiseLogicalOperation createGeneric(ASTNode ast, RNode left, Operation op, RNode right) {
        return new ElementwiseLogicalOperation(ast, left, op, right) {
            ...
            @Override public RAny execute(RAny leftValue, RAny rightValue) {
                if (leftValue instanceof RLogical && rightValue instanceof RLogical) { return op.op((RLogical) leftValue, (RLogical) rightValue, ast); }
                if ((leftValue instanceof RInt || leftValue instanceof RDouble || leftValue instanceof RLogical)
                        && (rightValue instanceof RInt || rightValue instanceof RDouble || rightValue instanceof RLogical)) { return op.op(leftValue.asLogical(), rightValue.asLogical(), ast); }
                if (leftValue instanceof RRaw && rightValue instanceof RRaw) { return op.op(leftValue.asRaw(), rightValue.asRaw(), ast); }
    ...
    public static class Specialized extends ElementwiseLogicalOperation {
        ...
        public static ElementwiseLogicalOperation create(final ASTNode ast, RNode left, final Operation op, RNode right, RAny leftTemplate, RAny rightTemplate) {
            if (leftTemplate instanceof ScalarLogicalImpl && rightTemplate instanceof ScalarLogicalImpl) { return new Specialized(ast, left, op, right, new Action() {
                @Override RLogical doFor(RAny leftValue, RAny rightValue, ASTNode ast) throws UnexpectedResultException {
            ...
            if (leftTemplate instanceof RLogical && rightTemplate instanceof RLogical) { return new Specialized(ast, left, op, right, new Action() {
                @Override RLogical doFor(RAny leftValue, RAny rightValue, ASTNode ast) throws UnexpectedResultException {
                    if ((leftValue instanceof RLogical && rightValue instanceof RLogical)) { return op.op((RLogical) leftValue, (RLogical) rightValue, ast); }
                    throw new UnexpectedResultException(null);
                }
            }); }
            if (leftTemplate instanceof RRaw && rightTemplate instanceof RRaw) { return new Specialized(ast, left, op, right, new Action() {
                @Override RRaw doFor(RAny leftValue, RAny rightValue, ASTNode ast) throws UnexpectedResultException {
                    if ((leftValue instanceof RRaw && rightValue instanceof RRaw)) { return op.op((RRaw) leftValue, (RRaw) rightValue, ast); }
                    throw new UnexpectedResultException(null);
                }
            }); }
    ...
    public abstract static class Operation {
        ...
        public RRaw op(RRaw a, RRaw b, ASTNode ast) {
            ...
            int[] dimensions = Arithmetic.resultDimensions(ast, a, b);
            Names names = Arithmetic.resultNames(ast, a, b);
            if (na == 0 || nb == 0) { return RRaw.EMPTY; }
    ...
    public static final Operation AND = new Operation() {
        @Override public byte op(byte a, byte b) {
            return (byte) (a & b);
        }

        @Override public int op(int a, int b) {
            if (a == RLogical.TRUE) { return b; }
            if (a == RLogical.FALSE) { return RLogical.FALSE; }
            // a == RLogical.NA
            if (b == RLogical.TRUE) {
                return RLogical.NA;
            } else {
    ...
    public static final Operation OR = new Operation() {
        ...
        @Override public int op(int a, int b) {
            if (a == RLogical.TRUE) { return RLogical.TRUE; }
            if (a == RLogical.FALSE) { return b; }
            // a == RLogical.NA
            if (b == RLogical.TRUE) {
