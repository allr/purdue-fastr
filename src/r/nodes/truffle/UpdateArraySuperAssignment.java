...
import r.nodes.truffle.UpdateArrayAssignment.AssignmentNode;
 ...
 * execution checks the frame to be not null. The const / non-const distinction is done in the create method as in
 * UpdateVariable and no rewrite on runtime is used for this.
 */
public class UpdateArraySuperAssignment extends BaseR {
    ...
    /**
     * Frame resolved (not null) version of the non const super assignment. Obtains the lhs, executes the assignment and
     * if the result differs, writebacks the new value.
     */
    ...
    /**
     * Const super assignment. Checks the frame is not null, then evaluates the rhs once and converts to the const
     * resolved node.
     */
