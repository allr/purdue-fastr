...
public class FusedOperator extends View.Visitor {
    ...
    /**
     * Prototype of fused operators. Contains stubs of the methods used to bind the operator to particular
     */
    public static abstract class Prototype extends View.Visitor {
         ...
         * collisions that may otherwise happen. During the binding of the fused operator when the view tree is walked,
         * all inner nodes are checked if they confirm to the given classes. For certain inner nodes more than one slot
         * of the array may be used.
         */
        ...
        /**
         * If the fused operator is bound to a certain view, the view is also hold in this variable so that if the view
         * gets rebound to another view the binding can be removed.
         */
        protected View boundView = null;

        /**
         * Checks next class in the array of inner node classes. If the class does not correspond or no classes are left
         * the NotSupported error is raised. Otherwise the index to the node classes is automatically advanced.
         */
        protected void checkClass(Class clazz) {
            if ((nodeClassesIndex == nodeClasses.length) || (nodeClasses[nodeClassesIndex] != clazz)) throw new NotSupported();
            ++nodeClassesIndex;
        }

        /**
         * Any view that is not explicitely listed in a method below is not understood by the fused operator. Default
         * case throws the NotSupported exception.
         */
        ...
        /**
         * Any leaf type that is not especially overriden is not understood by the fused operator. Default case throws
         * the NotSupported exception. Since the leaf nodes should bind the inputs to the fields of the fused operators,
         * they must be generated in the children and cannot be listed here.
         */
        @Override public void visitLeaf(RAny value) {
            assert (false);
            throw new NotSupported();
        }
        ...
        /**
         * The visitor methods for understood operators. Each of such classes perform first the class checks to make
         * sure the view signature is the same and then visits its children so that the whole view is visited.
         */
        ...
        /**
         * This is overriden by the generated code to patch the AST nodes of the views that may emit warnings correctly.
         */
        ...
        protected final void unbind() {
            ...
            if (boundView != null) boundView.bind(NO_FUSION);
            boundView = null;
        }

        /**
         * Binds the fused operator to given view. The view is walked and checked that it is the same as the one for
         * which the fused operator was created by checking classes of inner nodes and their operations. Input types are
         * checked and the inputs are bound to the local input variables. TODO calculate dimensions attributes and names
         */
        public void bind(View view) {
            ...
            try {
                ...
                boundView = view;
                view.visit(this);
                if (nodeClassesIndex != nodeClasses.length) throw new NotSupported();
            } catch (Error e) {
                free();
                throw e;
            }
        ...
        /**
         * Frees the fused operator from the previously bound inputs. This method should be overriden to set all inputs
         * of the fused operator to null so that they can be garbage collected if no-one holds their references.
        ...
        /**
         * Computes the materialization of the previously bound view. This is the main method to be overriden in the
         * subclasses.
         * 
    ...
    /**
     * Index of the input whose size the result has. This variable propagates through the tree linearly and must be
     * captured by the branch points. At the end of the execution it will hold the index of the input that determines
     * the size of the materialized result.
     */
    int resultSize = 0;
    /**
     * Variable in which the result is stored. This vaariable propagates through the tree linearly and must be captured
     * by the branch points. At the end of the execution it will hold the variable in which the result will be stored.
     */
    String resultVar;
    /**
     * Type of the result. Determined by the view nodes and updates as the tree is visited. At the end, the type of the
    ...
    /**
     * Number of AST variables to be created in the fused view. These are needed for the proper displaying of the
     * warnings (int overflow,...).
     */
    int astVariables = 0;
    ...
    @Override public String toString() {
        ...
        for (Input i : inputs) {
            ...
            if (i.type == Fusion.DOUBLE) sb.append("double  ");
            else if (i.type == Fusion.INT) sb.append("int     ");
            else if (i.type == Fusion.COMPLEX) sb.append("complex ");
    ...
    private void addFreeMethod() throws CannotCompileException {
        ...
        for (Input i : inputs)
            ...
            case Fusion.INT:
            case Fusion.DOUBLE:
                if (i.isVector) sb.append("    " + i.inputName() + " = null;\n");
                break;
            }
        sb.append("}\n");
        fop.addMethod(CtNewMethod.make(sb.toString(), fop));
    ...
    private void addGetElementMethod() throws CannotCompileException {
        ...
        for (Input i : inputs)
            if (i.isVector && i.increment == Input.Increment.CUSTOM_INDEX) sb.append(", int i" + i.index);
        sb.append(") {\n");
        appendCode(sb);
    ...
    private void addGetIntMethod() throws CannotCompileException {
        ...
        if (resultType == Fusion.INT) {
            ...
            for (Input i : inputs)
                if (i.isVector && i.increment == Input.Increment.CUSTOM_INDEX) sb.append("    int i" + i.index + " = index % input" + i.index + ".length;\n");
            sb.append("    return getElementInt(index");
            for (Input i : inputs)
                if (i.isVector && i.increment == Input.Increment.CUSTOM_INDEX) sb.append(", i" + i.index);
            sb.append(");\n");
            sb.append("}\n");
        } else {
    ...
    private void addMaterializeMethod() throws CannotCompileException {
        ...
        for (Input i : inputs)
            if (i.isVector && i.increment == Input.Increment.CUSTOM_INDEX) sb.append("    int i" + i.index + " = 0;\n");
        // main loop header
    ...
    /**
     * Adds given class to the list of classes that will be checked at runtime when binding the fused operator. Note
     * that this must be the same order the classes will later be checked.
     */
    ...
    private void binaryDDtoD(String left, String right, Arithmetic.ValueArithmetic arit) {
        ...
        if (arit == Arithmetic.ADD) code.append("        " + resultVar + " = " + left + " + " + right + ";\n");
        else if (arit == Arithmetic.SUB) code.append("        " + resultVar + " = " + left + " - " + right + ";\n");
        else if (arit == Arithmetic.MULT) code.append("        " + resultVar + " = " + left + " * " + right + ";\n");
        else if (arit == Arithmetic.DIV) code.append("        " + resultVar + " = " + left + " / " + right + ";\n");
        else if (arit == Arithmetic.MOD) code.append("        " + resultVar + " = " + left + " % " + right + ";\n");
        else {
            assert (false);
            throw new NotSupported();
     ...
     */
    @Override public void visitLeaf(RAny input) {
        assert (false);
        throw new NotSupported();
    }
    ...
    @Override public void visitDoubleLeaf(RDouble data) {
        Input i = new Input(data, inputs.size(), Fusion.DOUBLE, inputIsVector);
        if (subsetIndex != null) i.setSubsetIndexed(subsetIndex);
        else if (isResultSize) resultSize = i.index;
        else i.setCustomIndexed();
        inputs.add(i);
    ...
    @Override public void visitIntLeaf(RInt data) {
    ...
    @Override public void visitIntSequenceLeaf(IntImpl.RIntSequence data) {
    ...
    @Override public void visitComplexLeaf(RComplex data) {
    ...
    static class NoFusion extends Prototype {
        ...
        @Override public RArray materialize_(View view) {
            return view.materialize_();
        }

        @Override public int getInt(View view, int index) {
            return view.getInt_(index);
        }

        @Override public double getDouble(View view, int index) {
