...
public class Fusion {
    ...
    /**
     * Materializes the given view. Computes the hash of the view to determine if the view has already been compiled. If
     * found, obtains the cached fused operator, otherwise builds a new fused operator for the view. The fused operator
     * is then executed to materialize the view. Note that if anything goes wrong a special fused operator that only
     * calls the classic materialize in the view.
     * 
     * @param view
     *            View to be materialized
    ...
    public static RArray materialize(View view) {
        ...
        if (fusedOperator == null) {
            if (ENABLE_STATISTICS) ++materialize_hashed;
            int hash = Hash.view(view);
            if (hash == 0) {
                if (ENABLE_STATISTICS) ++materialize_hashFailed;
        ...
        if (VERIFY) verify(result, view);
        return result;
    }

    public static int getInt(View view, int index) {
        if (ENABLE_STATISTICS) ++getInt;
        FusedOperator.Prototype fusedOperator = view.boundFusedOperator();
        if (fusedOperator == null) {
            if (ENABLE_STATISTICS) ++getInt_hashed;
            int hash = Hash.view(view);
            if (hash == 0) {
                if (ENABLE_STATISTICS) ++getInt_hashFailed;
                FusedOperator.NO_FUSION.bind(view); // bind to NO_FUSION if we ever use it again
                return view.getInt_(index);
            }
            fusedOperator = getOrCompileFusedOperator(view, hash);
            if (ENABLE_STATISTICS && fusedOperator.boundView != null) ++getInt_rebound;
            fusedOperator.bind(view);
        } else {
            if (fusedOperator.boundView != null && fusedOperator.boundView != view) {
                if (ENABLE_STATISTICS) ++getInt_rebound;
                fusedOperator.bind(view);
            }
        }
        ...
        if (VERIFY) {
            ...
            if (result != check) throw new Error("FUSION: elements differ");
        }
        return result;
    }

    public static double getDouble(View view, int index) {
        if (ENABLE_STATISTICS) ++getDouble;
        FusedOperator.Prototype fusedOperator = view.boundFusedOperator();
        if (fusedOperator == null) {
            if (ENABLE_STATISTICS) ++getDouble_hashed;
            int hash = Hash.view(view);
            if (hash == 0) {
                if (ENABLE_STATISTICS) ++getDouble_hashFailed;
                FusedOperator.NO_FUSION.bind(view); // bind to NO_FUSION if we ever use it again
                return view.getDouble_(index);
            }
            fusedOperator = getOrCompileFusedOperator(view, hash);
            if (ENABLE_STATISTICS && fusedOperator.boundView != null) ++getDouble_rebound;
            fusedOperator.bind(view);
        } else {
            if (fusedOperator.boundView != null && fusedOperator.boundView != view) {
                if (ENABLE_STATISTICS) ++getDouble_rebound;
                fusedOperator.bind(view);
            }
        }
        ...
        if (VERIFY) {
            ...
            if (result != check && !Double.isNaN(result) && !Double.isNaN(check)) throw new Error("FUSION: elements differ");
        }
        return result;
    }
    ...
    public static FusedOperator.Prototype getOrCompileFusedOperator(View view, int hash) {
        ...
        if (fusedOperator == null) {
            if (ENABLE_STATISTICS) ++compiled;

            fusedOperator = FusedOperator.build(view, hash);
            operators.put(hash, fusedOperator);
            if (ENABLE_STATISTICS && fusedOperator == FusedOperator.NO_FUSION) ++compilationFailed;
    ...
    public static void verify(RArray result, View view) {
        ...
        if (result.getClass() != check.getClass()) throw new Error("FUSION: different class types");
        if (result.size() != check.size()) throw new Error("FUSION: different result sizes");
        if (result instanceof RDouble) {
            double[] r = ((RDouble) result).getContent();
            double[] c = ((RDouble) check).getContent();
            for (int i = 0; i < r.length; ++i)
                if ((r[i] != c[i]) && !Double.isNaN(r[i]) && !Double.isNaN(c[i])) throw new Error("FUSION: elements differ");
        } else if (result instanceof RInt) {
            int[] r = ((RInt) result).getContent();
            int[] c = ((RInt) check).getContent();
            for (int i = 0; i < r.length; ++i)
                if (r[i] != c[i]) throw new Error("FUSION: elements differ");
        } else {
