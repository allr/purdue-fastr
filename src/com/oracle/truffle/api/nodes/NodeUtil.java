...
public class NodeUtil {
    ...
    /**
     * Interface that allows the customization of field offsets used for {@link Unsafe} field accesses.
     */
    ...
    /**
     * Information about a {@link Node} class. A single instance of this class is allocated for every subclass of
     * {@link Node} that is used.
     */
        ...
        private Node nodeAt(int idx) {
            ...
            if (idx < nodeCount) {
                ...
                for (long fieldOffset : nodeClass.childrenOffsets) {
                    ...
                    if (idx < nodeCount + nodeArray.length) { return nodeArray[idx - nodeCount]; }
    ...
    @SuppressWarnings("unchecked") public static <T extends Node> T cloneNode(T orig) {
        ...
        for (long fieldOffset : nodeClass.childOffsets) {
            ...
            if (child != null) {
                ...
                if (clonedChild == null) { return null; }
    ...
    private static Field[] getAllFields(Class<? extends Object> clazz) {
        ...
        if (clazz.getSuperclass() != null) { return concat(getAllFields(clazz.getSuperclass()), declaredFields); }
    ...
    public static <T extends Annotation> T findAnnotation(Class<?> clazz, Class<T> annotationClass) {
        if (clazz.getAnnotation(annotationClass) != null) {
            ...
            for (Class<?> intf : clazz.getInterfaces()) {
                if (intf.getAnnotation(annotationClass) != null) { return intf.getAnnotation(annotationClass); }
            }
    ...
    @SuppressWarnings("unchecked") public static <T extends Node> T findParent(final Node start, final Class<T> clazz) {
        assert start != null;
    ...
    @SuppressWarnings("unchecked") public static <I> I findParentInterface(final Node start, final Class<I> clazz) {
        assert start != null;
        if (clazz.isInstance(start.getParent())) {
    ...
    @SuppressWarnings("unchecked") public static <T> T findFirstNodeInstance(Node root, Class<T> clazz) {
    ...
    public static <T extends Node> List<T> findAllNodeInstances(final Node root, final Class<T> clazz) {
        ...
        root.accept(new NodeVisitor() {
            ...
            @SuppressWarnings("unchecked") @Override public boolean visit(Node node) {
    ...
    public static <T extends Node> List<T> findNodeInstancesInFunction(final Node root, final Class<T> clazz) {
        ...
        root.accept(new NodeVisitor() {
            ...
            @SuppressWarnings("unchecked") @Override public boolean visit(Node node) {
                if (clazz.isInstance(node)) {
                    nodeList.add((T) node);
                } else if (node instanceof RootNode && node != root) { return false; }
                return true;
            }
        });
        return nodeList;
    ...
    private static final class NodeCountVisitor implements NodeVisitor {
        ...
        @Override public boolean visit(Node node) {
            if (node instanceof RootNode && node != root) { return false; }
